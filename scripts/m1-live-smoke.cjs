...
async function main() {
  ...
  if ([stopScenario, denyScenario, steerScenario].filter(Boolean).length > 1) throw new Error('一次只执行一个验收场景');
  ...
  try {
    ...
    await page.getByRole('textbox', { name: '任务要求' }).fill(denyScenario
    ...
    apiKey = undefined;
    try { await fs.writeFile(path.join(run, 'summary.json'), JSON.stringify({ model, scenario: steerScenario ? 'steer' : denyScenario ? 'deny' : stopScenario ? 'stop' : 'first-turn', submissionAttempts, passed, approvalsObserved, stage,
      steerBinding,
      ...
      scope: steerScenario ? '真实运行中补充和同轮次产出；不代替允许、历史和退出验收' : denyScenario ? '真实原生审批拒绝和目标未写入；不代替允许、补充、历史和退出验收' : stopScenario ? '真实运行中停止和延迟副作用；不代替补充、拒绝、历史和退出验收' : '首次真实执行；不代替审批允许/拒绝、补充、停止、历史和退出完整验收' }, null, 2), 'utf8');
