...
test('草稿迁移：v6 一致性备份保留发送意图，新表创建后原任务不丢失', async () => {
  ...
  try { previous.exec('ALTER TABLE tasks DROP COLUMN organization_revision; DROP TABLE runtime_leases; DROP TABLE drafts; DROP INDEX execution_intents_turn; ALTER TABLE execution_intents DROP COLUMN turn_id; PRAGMA user_version=6'); } finally { previous.close(); }
