...
test('发送意图迁移：v5 项目保留，升级前快照仍是可读取的 v5', async () => {
  ...
  try { assert.equal(current.prepare('PRAGMA user_version').get().user_version, 14); }
