...
test('轮次落盘：错误操作或 thread 不能确认任务，失败不留下半条绑定', async () => {
  ...
  assert.deepEqual(readWorkspace(root).tasks, [{ ...task, archivedAt: null, pinnedAt: null, organizationRevision: 0, threadId: 'thread-1' }]);
  ...
  assert.deepEqual(readWorkspace(root).tasks, [{ ...task, archivedAt: null, pinnedAt: null, organizationRevision: 0, executionState: 'running', threadId: 'thread-1', turnId: 'turn-1' }]);
});
...
test('发送后失联：意图与任务原子进入核对，旧操作不能重新发送或改写其他任务', async () => {
  ...
  assert.deepEqual(readWorkspace(root).tasks, [{ ...task, archivedAt: null, pinnedAt: null, organizationRevision: 0, executionState: 'reconciling', observedAt }]);
  assert.throws(() => markSubmissionDispatched(root, task.taskId, intent.operationId));
...
test('发送意图迁移：v5 项目保留，升级前快照仍是可读取的 v5', async () => {
  ...
  try { previous.exec('ALTER TABLE tasks DROP COLUMN archived_at; ALTER TABLE tasks DROP COLUMN pinned_at; ALTER TABLE tasks DROP COLUMN organization_revision; DROP TABLE runtime_leases; DROP TABLE drafts; DROP TABLE execution_intents; PRAGMA user_version=5'); } finally { previous.close(); }
  ...
  try { assert.equal(current.prepare('PRAGMA user_version').get().user_version, 12); }
