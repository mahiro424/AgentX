...
test('v8 升级保留任务与意图的精确关联，旧版本快照可读取，不伪造历史进程归属', async () => {
  ...
  try { database.exec('DROP TABLE input_materials; DROP TABLE materials; ALTER TABLE drafts DROP COLUMN material_ids; ALTER TABLE tasks DROP COLUMN archived_at; ALTER TABLE tasks DROP COLUMN pinned_at; ALTER TABLE tasks DROP COLUMN organization_revision; DROP TABLE runtime_leases; PRAGMA user_version=8'); } finally { database.close(); }
