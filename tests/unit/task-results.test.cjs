...
test('v7 迁移：原会话、意图和一致性备份保留，缺失的轮次关联不按时间猜测', async () => {
  ...
  try { database.exec('DROP TABLE runtime_leases; DROP INDEX execution_intents_turn; ALTER TABLE execution_intents DROP COLUMN turn_id; PRAGMA user_version=7'); }
  ...
  const backups = (await fs.readdir(value.root)).filter(name => /^agentx\.before-v9\..+\.db$/.test(name));
