...
async function v12(corrupt = false) {
  ...
  const now = new Date().toISOString(), scope = { projectId, taskId };
  ...
  try {
    ...
    db.prepare('INSERT INTO tasks VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)').run(taskId,corrupt ? randomUUID() : projectId,'旧会话',root,now,now,now,'completed','v12-thread','v12-turn',7,now,null);
    ...
    db.prepare('INSERT INTO drafts VALUES(?,?,?,?,?)').run(JSON.stringify([projectId,taskId]),projectId,taskId,'尚未发送的修改',9);
    db.prepare('INSERT INTO runtime_leases VALUES(?,?,?,?,?,?,?,?,?,?)').run(leaseId,instanceId,taskId,operationId,projectId,JSON.stringify(identity),now,1,now,now);
  } finally { db.close(); }
  return { root, filename, projectId, taskId, operationId, leaseId, scope, now, identity };
}

test('v12→v13：有数据的项目、组织修订、草稿、意图和租约完整保留，备份仍可读', async () => {
  const f = await v12();
  ...
  assert.deepEqual(readDraft(f.root,f.scope),{...f.scope,text:'尚未发送的修改',revision:9});
  ...
  const backups=(await fs.readdir(f.root)).filter(name=>/^agentx\.before-v13\..+\.db$/.test(name)); assert.equal(backups.length,1);
  ...
  try { assert.equal(after.prepare('PRAGMA user_version').get().user_version,13); assert.deepEqual(after.prepare('PRAGMA foreign_key_check').all(),[]); }
  finally {after.close();}
});
...
test('v12 引用损坏：迁移失败保留旧库和备份，不清库、不提交半次表重建', async () => {
  ...
  assert.equal((await fs.readdir(f.root)).filter(name=>name.startsWith('agentx.before-v13.')).length,1);
});
