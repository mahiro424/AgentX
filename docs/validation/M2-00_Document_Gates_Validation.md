...
上述是初始文档提交的检查快照。PR #26 在 head 951602c 上的 PR Windows 检查通过，但 push 检查有三个桌面失败，原始记录保留在[输入修复验证](M2_Input_Caret_Fix_Validation.md)。不能靠另一次绿色覆盖失败。

输入竞态经独立 [#27](https://github.com/mahiro424/AgentX/issues/27) / [PR #28](https://github.com/mahiro424/AgentX/pull/28) 修复，完成真实旧包 RED、新包回归、实机和独立审查；push/PR Windows 均为 186/186，文档门禁均成功后，已于 2026-09-09 合入 m2 的 9ca4d6b6f86d79fe31e33095af5b8288cb3bf8f7，#27 已关闭。输入修复不混作 #20 的实现；#26 同步已经合入的 m2 后，业务源码与 m2 相同。原目录应用包由 #27 更新，不因文档同步重复覆盖。

本次同步仅合并两套 CI 分支清单的并集，保留 m2-ci-fix、m2-docs 及五个切片，不移除原门禁。仓库 fetch refspec 仅跟踪 master，首次使用 origin/m2 名称失败后，核验 FETCH_HEAD 精确等于 PR #28 合并 SHA 再快进本地 m2；未改变 fetch 配置、未强推或重置。冲突只在上述两个分支清单，已按并集解决。

#26 最新 head 仍需独立 Windows/文档检查成功与合入 m2，才能关闭 #20 并解锁 #21。完整 M2 业务功能没有因此提前完成。
