...
- 新增有限 `continueExecution`：必须携带原 task / thread / expectedTurnId，终态才可提交；重新获取当前模型配置快照，准备失败不偷偷沿用旧密钥或新建无关联 thread。
- 固定公开 `thread/read → thread/resume → turn/start`：恢复前核对最新轮次和完整历史，恢复后核对目录、Flash、用户审批、沙箱与 idle。产品以事务插入独立操作意图、清空当前 turn 绑定并等待新的确认，原任务标题和旧操作/基线保留。无补丁、无自动重放或失败重试。
- 新轮应答前也按已核验的旧轮 ID 忽略晚到事件；应答丢失或连接失败进入核对，停止按钮/审批语义沿用既有实现。每轮文件基线重新捕获，但旧基线不可覆盖；仍只对当前已结束轮次提供实际文件检查，不声称保存了旧轮结束时的原子文件快照。
- 输入框 Enter 在原会话开始新轮，运行时同位置仍为停止；Ctrl+Enter 换行、组合输入不提交。续轮准备失败保留草稿，成功后也不清掉提交期间继续输入的文字；切走后的迟到应答不抢导航。旧历史与当前轮在同一连续工作记录区滚动，缓存注明不是新轮终态，当前轮完整历史读回后不重复展示。
- 失败/中断仍能查看实际已知变化；面板明确“不代表目标完成或已撤销修改”，检查/关闭不改变终态。

### TDD 与桌面证据

- `93` 首个续轮方法缺失 RED → `94` GREEN；`95` 旧命令 delta 误判新轮上下文 RED → `96` 20/20 回归；`97` 新轮后端 7/7（新配置、原历史、协议不符、旧事件、未知应答、早到终态、事务回滚）。
- `98` 原会话输入禁用 RED → `100` GREEN；`101` 旧历史/新轮独立滚动 RED → `103` GREEN；`105` 串行桌面回归 20/20。
- `106` 失败/中断结果缺少部分产物说明 RED → `109` 结果与续轮桌面 12/12，含深浅色、960×640、连续记录、准备失败、迟到应答、有限 IPC。
- `104` 非仓库默认并发命令 19/20，其中 Electron 首次显示超时；相同代码按仓库既定 `--test-concurrency=1` 运行 `105` 20/20。不改写原失败，也不据此宣称已定位系统超时根因。
- `107` typecheck 通过；`108` 打包通过并更新指定目录。工作台 [浅色](m1-05/continue-light.png)、[深色](m1-05/continue-dark.png)、[960×640](m1-05/continue-compact.png) 已实际查看，保持原侧栏、连续记录和单一发送/停止主动作，不添加 V1 假入口。
- 独立只读后端审查未发现可复现 P1/P2。UI 审查提出无 turnId 的提交错误可能残留；点验发现该错误应保留于本次发送不确定的轮次，用户下一次显式发送开始时清理；`105/109` 已覆盖失败后再次成功及新输入保留，不以隐藏未决错误作为修复。旧停止/审批/补充错误仍绑定原 turnId。

本节尚待最终全量结果和真实 Flash 续轮证据；不能据局部测试关闭 #10。M1-06 与整体 M1 最终交付仍未完成。

## M1-05 切片最终本地验收

- `110-continue-full-tests.log`：**164/164**（322242.9763 ms），包含真实打包 Electron；typecheck、固定协议摘要及 `git diff --check` 通过。当前包为 `108-continue-partial-package.log`，固定路径已更新。
- 真实验收使用前批审批合成项目的同一 task/thread，通过工作台 Enter 发起两个新轮，不替换 Main、模型或历史接口。第一轮只修复 `math.cjs` / `text.cjs`，第二轮只为 `text.cjs` 增加 trim；两轮独立 `node --test acceptance.test.cjs` 均退出 0，额外空白输入断言退出 0。
- 原测试、人工文件、审批脚本和目录外 `allowed.txt` 的 SHA-256 前后相同；每次续轮前的完整旧轮记录与续轮后前缀逐字段一致。关闭后重新打开，公开历史返回 3 轮且无执行快照，不自动发送。
- 两轮各有一次沙箱内 `node --test` 子进程 `spawn EPERM`，记录退出 1；随后引擎提出原生审批。核对完整命令和合成目录后逐条允许，测试真实退出 0。没有扩大为会话级自动批准，不隐藏原失败。验收驱动 stdin 不可写，未重启驱动或重发轮次；通过既有测试 Electron 的本地调试连接调用同一有限产品审批桥完成回答。
- [脱敏现场摘要](m1-05/live-continuation-summary.json)、[两文件修复](m1-05/live-round-1-results.png)、[原会话追加要求](m1-05/live-round-2-results.png)。原始执行目录、加密凭据和完整报告不入库。第二批真实验收已用 **5/5**，余量 **0**；此后不得自动追加模型调用。
- 本地状态矩阵全部覆盖；系统 DPI / 中文输入法采用用户明确通过的既有验收，不冒称逐个新面板重新操作了系统设置。PR/CI/合入记录将在远程完成后补充。

M1-06 托盘、退出和异常核对仍未实现；完整 M1 尚未完成，不以本切片替代最终应用包与工单链路交付。
