# AgentX 状态流转与恢复契约 v0.3

日期：2026-09-09。状态：V1 执行契约不变，M1 与 M2-01 会话组织/搜索已交付；材料/预览正交状态及办公、异常收尾仍按 M2 分期实施。历史 G1 的有限验证不代表本文全部转换通过；M1 不提供完整人工收尾，M2 实际进度见正式工单，不用文档分期代替实现证据。
...
| 材料准备 | 产品 | checking / ready / changed / unavailable / blocked | 保存引用与版本；读取状态重开重查，ready 不等于模型已经读取 |
| 文件预览 | 产品视图 + 实际文件 | closed / loading / readable / stale / failed | 可保存标签引用和阅读位置，不把缓存正文当文件真相或引擎状态 |
...
以下是 M2 拟定内部语义，不是新的顶层任务状态或 Codex 协议字段；具体持久化字段按对应切片实现。

| 事件 | 必须处理 | 不改变的事实 |
| --- | --- | --- |
| 添加材料 | checking 后记录 ready、unavailable 或 blocked；取消不写入；按草稿修订保存 | 不创建引擎 thread，不称已上传或已被模型使用 |
| 文件变化/重新读取 | 原版本变为 changed，实际重新核验后更新；失败保留引用与原因 | 不改写已经派发的材料版本，不自动发送 |
| 有效提交 | 再次核验材料；与发送意图绑定实际版本后进入原 submitting 路径 | 未确认派发时不清除草稿；旧响应不覆盖新草稿 |
| 出现产物 | 关联来源操作/轮次，记录当前文件及独立核验结果；部分文件仍可检查 | 文件存在、解析成功或预览可读不等于目标达成 |
| 打开/刷新预览 | closed → loading → readable 或 failed；文件变化使旧内容 stale，晚到旧版本不得覆盖 | 不启动/停止任务，不覆盖原文件，不释放执行占用 |
| 检查后续改 | 用户明确发新轮，沿用原 thread；原件、旧轮来源与新版本区分 | 不续跑通用 checkpoint，不伪造原子回滚或用户验收状态 |

UI 中的 generatingText、validating、document、spreadsheet 等仅描述工具过程或阅读场景。执行的 completed、failed、interrupted 仍由对应权威轮次事实决定；产品可另外显示文件验证失败，不能用预览状态篡改轮次终态。
