...
export function HistoryTimeline({ history, onOpenOutput }: { history: TaskHistory; onOpenOutput?: (item: CommandItem, trigger: HTMLButtonElement) => void }) {
  return <section className="execution-timeline" aria-label="会话历史">
    {history.turns.map((turn, index) => <section key={turn.turnId} aria-label={`第 ${index + 1} 轮`}>
      ...
      <ExecutionTimeline items={turn.items} approvals={[]} pending={new Set()} canAnswer={false} active={false} onAnswer={() => {}} onOpenOutput={onOpenOutput} />
