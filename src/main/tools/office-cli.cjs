...
async function main(args) {
  ...
  if (args[0] === 'write' && args.length === 3) {
    ...
    const result = await (extension === '.docx' ? writeDocument : write)(spec, extension);
    return { ...await writeNew(output, result.bytes), validation: 'structure-only',
      ...(extension === '.docx' ? { paragraphCount: result.paragraphCount } : { sheets: result.sheets, formulaStatus: 'not-recalculated' }) };
  }
  throw new Error('用法：read <材料路径> <SHA-256> <新 JSON 文件名> 或 write <内容 JSON 路径> <新 CSV/XLSX/DOCX 文件名>');
}

// 由引擎既有命令执行拥有该进程；本入口不启动第二套执行循环。
const timeout = setTimeout(() => { process.stderr.write('办公工具超过 30 秒，进程终止；可能留下未验证的部分文件'); process.exit(1); }, 30000);
main(process.argv.slice(2)).then(result => process.stdout.write(JSON.stringify(result))).catch(error => {
  process.stderr.write(error instanceof Error ? error.message : '办公工具失败'); process.exitCode = 1;
