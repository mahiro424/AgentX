...
export async function readFilePreview(root: string, input: unknown): Promise<FilePreview> {
  if (input && typeof input === 'object' && !Array.isArray(input) && 'kind' in input && input.kind === 'result') {
    ...
    const base = { source, name: path.basename(filename), path: filename, taskId: artifact.taskId, turnId: artifact.turnId,
      operationId: artifact.operationId, version, currentVersion: null, text: null, spreadsheet: null, document: null, pdf: null, pdfData: null, observedAt: new Date().toISOString() };
    try {
    ...
    return { ...base, currentVersion: record.version, status: 'ready', text: current.text, spreadsheet: current.spreadsheet, document: current.document, pdf: current.pdf, pdfData: current.pdfData,
  ...
  return { source, name: material.name, path: material.path, status: unsupported ? 'unsupported' : material.status,
    ...
    version: material.version, currentVersion: observed.currentVersion, text: observed.text, spreadsheet: observed.spreadsheet, document: observed.document, pdf: observed.pdf, pdfData: observed.pdfData, observedAt: observed.observedAt,
...
export async function openFilePreview(root: string, input: unknown, host: { openPath(filename: string): Promise<string>; showItemInFolder(filename: string): void }) {
  ...
  if (input.action === 'open' && !['.txt', '.md', '.markdown', '.csv', '.xlsx', '.docx', '.pdf'].includes(path.extname(value.path).toLowerCase())) throw new Error('本阶段仅开放已核验文本、表格、DOCX 或文本 PDF 文件的本机打开');
  try {
