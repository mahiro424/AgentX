...
export async function readFilePreview(root: string, input: unknown): Promise<FilePreview> {
  if (input && typeof input === 'object' && !Array.isArray(input) && 'kind' in input && input.kind === 'result') {
    ...
    const base = { source, name: path.basename(filename), path: filename, taskId: artifact.taskId, turnId: artifact.turnId,
      operationId: artifact.operationId, version, currentVersion: null, text: null, spreadsheet: null, document: null, pdf: null, pdfData: null, image: null, observedAt: new Date().toISOString() };
    try {
    ...
    const current = await inspectMaterialFile(filename, true, true), { record } = current;
    const readable = record.status === 'ready' || (record.status === 'blockedImage' && !!current.image);
    if (record.path !== filename || !readable || record.version?.sha256 !== version.sha256 || record.version.size !== version.size) {
      return { ...base, currentVersion: record.version, status: readable ? 'changed' : record.status,
        message: readable ? '产物内容已变化；此标签仍关联原观察版本，请重新检查文件改动以查看新版本' : record.message };
    }
    return { ...base, currentVersion: record.version, status: 'ready', text: current.text, spreadsheet: current.spreadsheet, document: current.document, pdf: current.pdf, pdfData: current.pdfData, image: current.image,
      message: current.spreadsheet ? '实际表格与已登记版本一致；公式未重算，业务数值需核对' : (current.document || current.pdf) ? '实际文档与已登记版本一致；内容与业务结果仍需核对' : '实际文本与已登记版本一致；变化不全部归因于 Agent' };
  }
  ...
  const unsupported = material.status === 'ready' && !['text', 'spreadsheet', 'document', 'pdf'].includes(material.kind);
  return { source, name: material.name, path: material.path, status: observed.image ? 'ready' : unsupported ? 'unsupported' : material.status,
    message: observed.image ? '仅本地图片预览；图像能力尚未验证，含图发送已阻断' : unsupported ? '此对象不提供文本预览；目录引用不会自动展开' : material.status === 'ready' ? '只读内容；不执行脚本或重算公式' : material.message,
    version: material.version, currentVersion: observed.currentVersion, text: observed.text, spreadsheet: observed.spreadsheet, document: observed.document, pdf: observed.pdf, pdfData: observed.pdfData, image: observed.image, observedAt: observed.observedAt,
...
export async function openFilePreview(root: string, input: unknown, host: { openPath(filename: string): Promise<string>; showItemInFolder(filename: string): void }) {
  ...
  if (input.action === 'open' && !['.txt', '.md', '.markdown', '.csv', '.xlsx', '.docx', '.pdf', '.png', '.jpg', '.jpeg', '.webp', '.gif'].includes(path.extname(value.path).toLowerCase())) throw new Error('本阶段仅开放已核验文本、表格、文档和可预览图片的本机打开');
  try {
