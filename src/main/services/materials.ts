...
export async function inspectMaterialFile(filename: string, parseOffice = true, previewImage = false): Promise<{ record: Omit<MaterialRecord, 'materialId'>; text: string | null; spreadsheet: Spreadsheet | null; document: OfficeDocument | null; pdf: PdfDocument | null; pdfData: string | null; image: ImagePreview | null }> {
  ...
  try {
    ...
    const kind = ['.txt', '.md', '.markdown'].includes(extension) ? 'text' : ['.csv', '.xlsx'].includes(extension) ? 'spreadsheet' : extension === '.docx' ? 'document' : extension === '.pdf' ? 'pdf' : ['.png', '.jpg', '.jpeg', '.webp', '.gif', '.bmp'].includes(extension) ? 'image' : 'unsupported';
    record = { ...record, kind };
    if (kind === 'unsupported') return result({ ...record, status: 'unsupported', message: '当前开放 TXT / Markdown、CSV / XLSX、DOCX 与文本 PDF；此格式暂不能发送' });
    const maxBytes = kind === 'image' ? MATERIAL_LIMITS.imageBytes : kind === 'spreadsheet' ? MATERIAL_LIMITS.spreadsheetBytes : ['document', 'pdf'].includes(kind) ? MATERIAL_LIMITS.documentBytes : MATERIAL_LIMITS.textBytes;
    if (initial.size > BigInt(maxBytes)) return result({ ...record, message: `材料超限：${kind === 'image' ? '图片最多 8 MiB' : kind === 'spreadsheet' ? '表格最多 8 MiB' : ['document', 'pdf'].includes(kind) ? '文档最多 8 MiB' : '文本最多 1 MiB'}` });
    ...
    try {
      ...
      if (['spreadsheet', 'document', 'pdf'].includes(kind) && parseOffice) {
        try { if (kind === 'spreadsheet') spreadsheet = await readSpreadsheet(bytes, extension); else if (kind === 'document') document = await readDocument(bytes, extension); else pdf = await readPdf(bytes, extension); }
...
export class MaterialService {
  ...
  async check(ids: unknown): Promise<MaterialRecord[]> {
    ...
    return Promise.all(records.map(async record => {
      ...
      return ['spreadsheet', 'document', 'pdf'].includes(record.kind) && compared.status === 'ready' && record.status !== 'ready' ? { ...compared, status: record.status, message: record.message } : compared;
    }));
  }
